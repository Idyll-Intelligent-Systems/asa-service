...
router.get('/:slug', asyncHandler(async(req, res) => {
  ...
  try {
    ...
    const creatureQuery = `
      SELECT c.*, t.method, t.preferred_food, t.kibble, 
             t.taming_time, t.torpor_depletion, t.food_consumption
      FROM creatures c
      LEFT JOIN taming_methods t ON c.id = t.creature_id
      WHERE c.slug = $1
    `;
    ...
    res.json({
      ...
      data: {
        creature: slug,
        name: creature.name,
        tameable: creature.tameable,
        method: creature.method,
        preferred_food: creature.preferred_food ? creature.preferred_food.split(',') : [],
        kibble: creature.kibble,
        taming_time: creature.taming_time,
        torpor_depletion: creature.torpor_depletion,
        food_consumption: creature.food_consumption
      }
    });
  } catch (error) {
...
router.get('/', asyncHandler(async(req, res) => {
  ...
  if (!db) {
    return res.json({
      success: true,
      data: Object.values(mockTamingData),
      message: 'Using mock data - database not connected'
    });
  }
  ...
  try {
    const query = `
      SELECT c.slug, c.name, c.tameable, t.method, t.preferred_food, t.kibble
      FROM creatures c
      LEFT JOIN taming_methods t ON c.id = t.creature_id
      WHERE c.tameable = true
      ORDER BY c.name
    `;
